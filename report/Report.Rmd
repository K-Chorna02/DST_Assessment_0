---
title: "Final_report"
author: "Minnie, Belle, Kateryna , Jemma, Gracie"
date: "2025-10-06"
output:
  html_document:
    toc: true
...
## Initial Model Fitting for PM₂.₅ Concentration
...
## Extending the Model with type_of_station as a Numeric Predictor
...
## Extending the Model with Who_region
...
## Applying the Model to Y_2 = PM_10 Concentration
...
## Applying the model to Y_3 = N0_2 Concentration 
...
ggplot(avg_con_long, aes(x = pop_bracket, y = avg_concentration, fill = pollutant)) +
...
Now we can see a trend in the data- for all the pollutants as the population size increases the concentration increases. This is something we would have expected as a higher population would mean more waste and pollution and hence worse air quality. We have also fit a straight linear line (green) and a loess line (red) which fits a smooth flexible curve to the model in order to highlight the general trend of the plots.

## Comparing models

We now compare the two models from the above graphs (linear model and the log transformed model) to determine statistically which model is best
...
# Section: Time series and NA analysis
...
# Section: Polutant concentration and coverage 
...
## Concentration over time by city
...
#Separating the data into regions
...
At a first glance comparing two different regions we can see that there are large difference in the quantity of data in, and how much data is missing from each region. Exploring this further we look at the proportion of missing data from each region, for each pollutant.
...
par(mfrow=c(1,3))
#par(pin = c(2,1.5))
barplot(height = pm10, names.arg = colnames(missing_vals), xlab = "WHO Region", ylab = "Percentage of missing data", main = "PM10 concentration missing data across regions")
barplot(height = pm25, names.arg = colnames(missing_vals), xlab = "WHO Region", ylab = "Percentage of missing data", main = "PM25 concentration missing data across regions")
barplot(height = no2, names.arg = colnames(missing_vals), xlab = "WHO Region", ylab = "Percentage of missing data", main = "NO2 concentration missing data across regions")

```
These bar plots show that PM10 concentration is the most consistently recorded pollutant overall, and no single region is well recorded across all three pollutants.


This section continues onto the Jupyter notebook file.
